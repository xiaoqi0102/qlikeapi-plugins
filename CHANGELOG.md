...
- **图床每行的启用/停用开关**：此前被表格里的 `table.tb input[type=checkbox]{width:15px;height:15px}` 规则
  一起压成了 15px 小方块，看起来像「单选圈」而不是开关。现在该规则排除 `.form-check-input`，
  每行恢复成和顶部「启用图床转换」总开关一样的胶囊+滑块（点一下立刻生效的语义不变）。
- **操作列统一口径**：表头「操作」与按钮**一律右对齐、与表格右边缘齐平**（站点余额 / 渠道实例 / 访问令牌 /
  渠道插件 / 图床）。此前表头没标题、按钮靠左、右侧留一截空白，看着像没做完。
  `table.tb td.acts` 也修回 `display:table-cell`（原被 `.acts{display:inline-flex}` 带跑，列宽行为异常）。
- **站点余额**：补上缺失的「操作」表头；**手工记账站点**未填余额时状态显示「待填余额」，
  不再计进「查询异常」计数（那不是查询失败，是还没填）。
- 清理两个探活残留站点（`zz-test-inline` / `zz-4xx-b`）。

## v3.16.1 — 2026-09-19

### 修：手工价更牢 —— 全局手工价也算「锁」（用户反馈：手工改的价会被上游同步覆盖）
- 上一版只保住了「填在渠道这一格」的手工价。如果用户把某个模型设成**全局手工价**（`model` + `*`），
  同步再写渠道专属价就会在页面上把它盖住 —— 等于用户填的价被同步改掉了。
- 现在 `balances._manual_lock(model, provider)` 两层都查：这一格是手工价、或这个模型有全局手工价，
  同步一律跳过，并在 `skipped` 里注明「（该模型设了全局手工价）」。
- 反例已测：全局价若是**同步来的**（`source != manual`）不算锁，渠道价照常写，不影响正常同步。
- 端到端实测（真实接口 + 真实同步，零成本）：渠道手工价 → 同步 `skipped=['gpt-image-2@aicost']`、
  库里仍是 `manual`、页面显示「手工」；全局手工价 → `skipped=['gpt-image-2@aicost（该模型设了全局手工价）']`、
  没写渠道专属价。核对完 `model_prices` 整表已还原。
